...
//nikolaos babis, aem:13, nikolaom@csd.auth.gr
//kyriaki mauropoulou, aem:12, mavropoul@csd.auth.gr
...
object Link_Prediction_Part1 {
  ...
  def Tf_Idf2(x: DataFrame, column: String, columnOut: String, numFeatures: Int): DataFrame = {
    ...
    //val duration = (System.nanoTime - t4) / 1e9d
    //println(duration)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //load training set
    val df1_2 = sparkSession.read.format("csv").option("header", "false").load("C:\\Users\\dell\\Desktop\\training_set.txt").toDF("info")
    //load testing set
    //println(df1_2.show())
    val df1_3 = sparkSession.read.format("csv").option("header", "false").load("C:\\Users\\dell\\Desktop\\testing_set_final.txt").toDF("info")
    ...
    def ProcessDataf(NodeInfo: DataFrame, NodeCombination: DataFrame): DataFrame = {
     // val t6 = System.nanoTime
      ...
      //val duration = (System.nanoTime - t6) / 1e9d
   ...
   // println(assembledFinalDf.show())
   // println(assembledTestDf.show())
...
/*
    ...
    println(s"F1 score decis. tree for 30% of our data = ${accuracy}")
    val training_all = dt.fit(dt_training_all)
    val dt_prediction = training_all.transform(dt_test)
    val dt_evaluator = new MulticlassClassificationEvaluator()
      .setLabelCol("label")
      .setPredictionCol("prediction")
      .setMetricName("f1")
    //println(dt_evaluator)
    val dt_accuracy = dt_evaluator.evaluate(dt_prediction)
    println(s"DT F1 score decision tree(all the file) = ${dt_accuracy}")
*/
   // val t7 = System.nanoTime
    ...
    //val duration = (System.nanoTime - t7) / 1e9d
   // println(duration)
...
/*
  ...
  val lr_trained = lr.fit(dt_training_all)
  val lr_prediction = lr_trained.transform(dt_test)
  val lr_evaluator = new MulticlassClassificationEvaluator()
    .setLabelCol("label")
    .setPredictionCol("prediction")
    .setMetricName("f1")
  val lr_accuracy = lr_evaluator.evaluate(lr_prediction)
  println(s"LR_F1 score = ${lr_accuracy}")
  */
  /*
    //Bayes
      val nb = new NaiveBayes()
      val nb_trained=nb.fit(dt_training_all)
      val nb_prediction=nb_trained.transform(dt_test)
      val nb_evaluator = new MulticlassClassificationEvaluator()
        .setLabelCol("label")
        .setPredictionCol("prediction")
        .setMetricName("f1")
      val nb_accuracy = nb_evaluator.evaluate(nb_prediction)
      println(s"NB_F1 score = ${nb_accuracy}")
*/
